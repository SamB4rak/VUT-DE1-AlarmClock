...
--! @brief Multiplexed driver for an 8-digit active-low 7-segment display.
--! @description
--! Displays the current time and alarm time on a Nexys A7-style 8-digit
--! 7-segment display. The module scans one digit at a time, decodes packed
--! BCD nibbles to segment patterns, controls decimal points as HH:MM colons,
--! and applies blink/blank effects based on the alarm-clock FSM state.
--!
--! Display layout:
--! - Digits 0..3 show current time as HH:MM.
--! - Digits 4..7 show alarm time as HH:MM.
--! - The time colon blinks using ce_1hz.
--! - The alarm colon is shown when the alarm is armed or being edited.
--!
--! State-dependent behavior:
--! - RUN: show current time and armed alarm normally.
--! - SET_TIME: blink the selected time digit.
--! - SET_ALARM: blink the selected alarm digit.
--! - ALARM_RING: blink the alarm half at the ce_blink rate.
--!
--! Relevant notes:
--! - State encoding must match fsm_ctrl.
--! - seg, an, and dp outputs are active low.
--! - G_MUX_MAX controls the display scan rate.
--!
